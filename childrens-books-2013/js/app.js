...
var nypl_banner_start = function(){
  window.setInterval(function(){
    nypl_banner_next();
  }, 7000);
};

var nypl_banner_next = function(){
  var $current = $('.header-image.active').first(),
      $all = $('.header-images').children('.header-image'),
      current_pos = $all.index($current),
      next_pos = current_pos + 1,
      $next;
  if ( next_pos >= $all.length ) next_pos = 0;
  $next = $all.eq(next_pos);
