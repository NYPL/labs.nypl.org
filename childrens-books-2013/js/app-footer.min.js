...
;(function(){var factory=function(signals){var hasher=(function(window){var POOL_INTERVAL=25,document=window.document,history=window.history,Signal=signals.Signal,hasher,_hash,_checkInterval,_isActive,_frame,_checkHistory,_hashValRegexp=/#(.*)$/,_baseUrlRegexp=/(\?.*)|(\#.*)/,_hashRegexp=/^\#/,_isIE=(!+"\v1"),_isHashChangeSupported=('onhashchange' in window)&&document.documentMode!==7,_isLegacyIE=_isIE&&!_isHashChangeSupported,_isLocal=(location.protocol==='file:');function _escapeRegExp(str){return String(str||'').replace(/\W/g,"\\$&");}function _trimHash(hash){if(!hash)return '';var regexp=new RegExp('^'+_escapeRegExp(hasher.prependHash)+'|'+_escapeRegExp(hasher.appendHash)+'$','g');return hash.replace(regexp,'');}function _getWindowHash(){var result=_hashValRegexp.exec(hasher.getURL());var path=(result&&result[1])||'';try{return hasher.raw?path:decodeURIComponent(path);}catch(e){return path;}}function _getFrameHash(){return _frame?_frame.contentWindow.frameHash:null;}function _createFrame(){_frame=document.createElement('iframe');_frame.src='about:blank';_frame.style.display='none';document.body.appendChild(_frame);}function _updateFrame(){if(_frame&&_hash!==_getFrameHash()){var frameDoc=_frame.contentWindow.document;frameDoc.open();frameDoc.write('<html><head><title>'+document.title+'</title><script type="text/javascript">var frameHash="'+_hash+'";</script></head><body>&nbsp;</body></html>');frameDoc.close();}}function _registerChange(newHash,isReplace){if(_hash!==newHash){var oldHash=_hash;_hash=newHash;if(_isLegacyIE)if(!isReplace)_updateFrame();else _frame.contentWindow.frameHash=newHash;hasher.changed.dispatch(_trimHash(newHash),_trimHash(oldHash));}}if(_isLegacyIE){_checkHistory=function(){var windowHash=_getWindowHash(),frameHash=_getFrameHash();if(frameHash!==_hash&&frameHash!==windowHash)hasher.setHash(_trimHash(frameHash));else if(windowHash!==_hash)_registerChange(windowHash);};}else _checkHistory=function(){var windowHash=_getWindowHash();if(windowHash!==_hash)_registerChange(windowHash);};function _addListener(elm,eType,fn){if(elm.addEventListener)elm.addEventListener(eType,fn,false);else if(elm.attachEvent)elm.attachEvent('on'+eType,fn);}function _removeListener(elm,eType,fn){if(elm.removeEventListener)elm.removeEventListener(eType,fn,false);else if(elm.detachEvent)elm.detachEvent('on'+eType,fn);}function _makePath(paths){paths=Array.prototype.slice.call(arguments);var path=paths.join(hasher.separator);path=path?hasher.prependHash+path.replace(_hashRegexp,'')+hasher.appendHash:path;return path;}function _encodePath(path){path=encodeURI(path);if(_isIE&&_isLocal)path=path.replace(/\?/,'%3F');return path;}hasher={VERSION:'1.2.0',raw:false,appendHash:'',prependHash:'/',separator:'/',changed:new Signal(),stopped:new Signal(),initialized:new Signal(),init:function(){if(_isActive)return;_hash=_getWindowHash();if(_isHashChangeSupported)_addListener(window,'hashchange',_checkHistory);else{if(_isLegacyIE){if(!_frame)_createFrame();_updateFrame();}_checkInterval=setInterval(_checkHistory,POOL_INTERVAL);}_isActive=true;hasher.initialized.dispatch(_trimHash(_hash));},stop:function(){if(!_isActive)return;if(_isHashChangeSupported)_removeListener(window,'hashchange',_checkHistory);else{clearInterval(_checkInterval);_checkInterval=null;}_isActive=false;hasher.stopped.dispatch(_trimHash(_hash));},isActive:function(){return _isActive;},getURL:function(){return window.location.href;},getBaseURL:function(){return hasher.getURL().replace(_baseUrlRegexp,'');},setHash:function(path){path=_makePath.apply(null,arguments);if(path!==_hash){_registerChange(path);if(path===_hash){if(!hasher.raw)path=_encodePath(path);window.location.hash='#'+path;}}},replaceHash:function(path){path=_makePath.apply(null,arguments);if(path!==_hash){_registerChange(path,true);if(path===_hash){if(!hasher.raw)path=_encodePath(path);window.location.replace('#'+path);}}},getHash:function(){return _trimHash(_hash);},getHashAsArray:function(){return hasher.getHash().split(hasher.separator);},dispose:function(){hasher.stop();hasher.initialized.dispose();hasher.stopped.dispose();hasher.changed.dispose();_frame=hasher=window.hasher=null;},toString:function(){return '[hasher version="'+hasher.VERSION+'" hash="'+hasher.getHash()+'"]';}};hasher.initialized.memorize=true;return hasher;}(window));return hasher;};if(typeof define==='function'&&define.amd)define(['signals'],factory);else if(typeof exports==='object')module.exports=factory(require('signals'));else window.hasher=factory(window.signals);}());
;(function($){$.fn.unveil=function(threshold,callback){var $w=$(window),th=threshold||0,retina=window.devicePixelRatio>1,attrib=retina?"data-src-retina":"data-src",images=this,loaded;this.one("unveil",function(){var source=this.getAttribute(attrib);source=source||this.getAttribute("data-src");if(source){this.setAttribute("src",source);if(typeof callback==="function")callback.call(this);}});function unveil(){var inview=images.filter(function(){var $e=$(this);if($e.is(":hidden"))return;var wt=$w.scrollTop(),wb=wt+$w.height(),et=$e.offset().top,eb=et+$e.height();return eb>=wt-th&&et<=wb+th;});loaded=inview.trigger("unveil");images=images.not(loaded);}var unveil_slow=_.throttle(unveil,500,{leading:false});$w.scroll(unveil_slow);$w.resize(unveil_slow);unveil_slow();return this;};})(window.jQuery||window.Zepto);
(function(){window.JST||(window.JST={});window.JST.book_modal=function(obj){var __p='';var print=function(){__p+=Array.prototype.join.call(arguments,'');};with(obj||{}){__p+='<div class="modal-header">\n    <!-- <a class="hide npr-logo" href="http://npr.org/books/"><img src="http://media.npr.org/chrome/books/npr-books-logo-color.png" alt="NPR Books" /></a> -->\n    <a href="#" class="hide mobile-dismiss" data-dismiss="modal" aria-hidden="true"><i class="icon-chevron-left"></i> Back to NYPL&rsquo;s Children&rsquo;s Books</a>\n    <button type="button" class="close" data-dismiss="modal" aria-hidden="true">&times;</button>\n</div>\n\n<div class="modal-body">\n    <div class="row">\n        <div class="modal-book-cover col-md-5">\n            <img id="book-image" src="img/cover/'+(book.slug)+'';if(SMALL_MOBILE){;__p+='-thumb';};__p+='.jpg" alt="'+(book.title)+'"/>\n        </div>\n\n        <div class="modal-text col-md-7">\n            <h4 class="modal-title" id="myModalLabel">'+(book.title)+'</h4>\n            ';if(book.author){;__p+='<p class="author">By <a style="color:#999; text-decoration: underline;" href="http://nypl.bibliocommons.com/search?t=author&q='+(book.author)+'&commit=Search&searchOpt=catalogue">'+(book.author)+'</a>';if(book.illustrator){;__p+='. Illustrated by '+(book.illustrator)+'';};__p+='</p>';};__p+='\n            ';if(book.genre){;__p+='<p class="genre"><strong>'+(COPY.content.genre)+' </strong>'+(book.genre)+'</p> ';};__p+='\n\n            <!-- Tags -->\n            ';if(book.tags.length>0){;__p+='<p class="tags"><strong>'+(COPY.content.tag_list)+' </strong>';_.each(book.tags,function(tag,index,list){;__p+='\n                <a\n                    class="btn btn-large btn-info tag"\n                    href="#/tag/'+tag+'">'+COPY.tags[tag]+'\n                </a>\n            ';});;__p+='</p> ';};__p+='\n\n            <!-- Book Description/Review -->\n            ';if(book.text){;__p+='<p class="text">'+(book.text)+'</p> ';};__p+='\n            ';if(book.reviewer){;__p+='<p class="reviewer">&mdash; '+(COPY.content.recommended_by)+' <strong>';if(book['reviewer link']){;__p+='<a href="'+book['reviewer link']+'" target="_blank">'+(book.reviewer)+'</a>';}else{;__p+=''+(book.reviewer)+'';};__p+='</strong>';if(book['reviewer ID']){;__p+=', '+book['reviewer ID']+'';};__p+='</p> ';};__p+='\n\n            <!-- Purchase/Related Links -->\n\n            <p class="book-modal-links">\n                ';if(book.text){;__p+='\n                  ';if(book.catalog_uri){;__p+='\n                    <a href="'+(book.catalog_uri)+'" target="_blank" class="purchase-link"><i></i> Check Out This Book!</a>\n                  ';}else{;__p+='\n                    <a href="http://nypl.bibliocommons.com/item/show/'+(book.biblio_id)+'" target="_blank" class="purchase-link"><i></i> Check Out This Book!</a>\n                  ';};__p+='\n                ';};__p+='\n                ';if(book.ebook){;__p+='\n                    <a href="'+(book.ebook)+'" target="_blank" class="purchase-link"><i></i> Read Ebook!</a>\n                ';};__p+='\n            </p>\n\n            <ul>\n                ';if(book.review_seamus_id){;__p+='\n                <li class="review_seamus_id book-modal-links">\n                <a href="http://npr.org/'+(book.review_seamus_id)+'" target="_blank"><strong>'+(COPY.content.npr_review_link)+'</strong> '+(book.review_seamus_headline)+'</a>\n                </li>\n                ';};__p+='\n\n                ';if(book.author_seamus_id){;__p+='\n                <li class="author_seamus_id book-modal-links">\n                    <a href="http://npr.org/'+(book.author_seamus_id)+'" target="_blank">\n                        <strong>'+(COPY.content.npr_author_link)+'</strong>\n                        ';if(book.author_seamus_headline){;__p+='\n                            '+(book.author_seamus_headline)+'\n                        ';}else{;__p+='\n                            '+(book.author)+'\n                        ';};__p+='\n                    </a>\n                </li>\n                ';};__p+='\n            </ul>\n\n        </div>\n    </div>\n</div>\n\n<div class="modal-footer">\n\n    ';if(previous!=null){;__p+='\n    <a href="#/book/'+previous+'" class="btn btn-default modal-nav-buttons" id="previous-book">PREVIOUS BOOK</a>\n    ';};__p+='\n\n    ';if(next!=null){;__p+='\n    <a href="#/book/'+next+'" class="btn btn-default modal-nav-buttons" id="next-book">NEXT BOOK</a>\n    ';};__p+='\n    <button type="button" class="btn btn-default" id="modal-close-btn" data-dismiss="modal">Close</button>\n\n</div>\n';}return __p;};})();
var MOBILE=Modernizr.touch;var SMALL=Modernizr.mq('only all and (max-width: 480px)');var $body;var $content;var $books_grid;var $all_tags;var $tags={};var $clear_tags;var $current_tag;var $modal;var $modal_content;var $books_list;var $back_to_top;var $mobile_filters_btn;var $filter;var $toggle_text;var $show_text_button;var $show_books_button;var $review;var next;var previous;var selected_tags=[];var first_hash=true;var scroll_to=function($el){var top=$el.offset().top;$body.scrollTop(top);};var back_to_top=function(){$body.scrollTo($content,{duration:450},'y');return false;};var isotope_grid=function(filter){$books_grid.isotope({filter:filter,transformsEnabled:!MOBILE,getSortData:{'id':function($el){return parseInt($el.data('sort'),10);}},sortBy:'id'});};var filter_books=function(){$all_tags.parent().removeClass('selected unavailable');$all_tags.removeClass('selected unavailable');if(selected_tags.length>0){var filter='';var label=[];for(var i in selected_tags){var slug=selected_tags[i];$tag=$tags[slug];$tag.addClass('selected');$tag.parent().addClass('selected');filter+='.tag-'+slug;label.push(COPY.tags[slug]);}label=label.join(', ');var remaining_books=_.filter(BOOKS,function(book){for(var i in selected_tags){var slug=selected_tags[i];if(_.indexOf(book.tags,slug)==-1)return false;}return true;});for(var tag_slug in COPY.tags){var has_results=false;for(var z=0;z<remaining_books.length;z++){var book=remaining_books[z];if(_.indexOf(book.tags,tag_slug)>=0){has_results=true;break;}}if(!has_results){$tag=$tags[tag_slug];$tag.parent().addClass('unavailable');$tag.addClass('unavailable');}}$clear_tags.removeClass('hide');$current_tag.find('#showing-span').text('Showing books tagged ');$current_tag.find('#tag-span').text(label);filter_books_list(filter);_.defer(isotope_grid,filter);}else{$clear_tags.addClass('hide');$current_tag.find('#showing-span').text('Showing all books');$current_tag.find('#tag-span').text('');filter_books_list(null);_.defer(isotope_grid,'*');}_.delay(function(){$(window).trigger('scroll');},1000);};var filter_books_list=function(filter){if(filter){$books_list.find('li').removeClass('visible');filter='';for(var i in selected_tags){var slug=selected_tags[i];filter+='.tag-'+slug;}$books_list.find(filter).addClass('visible');}else $books_list.find('li').addClass('visible');};var on_tag_clicked=function(){var slug=$(this).data('tag-slug');var already_selected=selected_tags.indexOf(slug);if(already_selected>=0)selected_tags.splice(already_selected,1);else selected_tags.push(slug);if(selected_tags.length>0)hasher.setHash('tag',selected_tags.join(','));else hasher.setHash('_');return false;};var on_page_scroll=function(){var y_scroll_pos=window.pageYOffset;var scroll_pos_test=1000;if(y_scroll_pos>scroll_pos_test&&$('#myModal:visible').length===0)$back_to_top.fadeIn(1000);else $back_to_top.fadeOut(1000);};var on_modal_tag_clicked=function(){back_to_top();return true;};var on_clear_tags_clicked=function(){hasher.setHash('_');return false;};var on_tag_hash=function(tags){selected_tags=tags.split(',');filter_books();};var on_book_hash=function(slug){$modal_content.empty();book=_.find(BOOKS,function(book){return book.slug==slug;});var grid_item;if($books_grid.is(':visible')){grid_item=$('#'+book.slug);next=grid_item.nextAll(':not(.isotope-hidden)').first();previous=grid_item.prevAll(':not(.isotope-hidden)').first();if(next.length===0)next=null;else next=next.attr('id');if(previous.length===0)previous=null;else previous=previous.attr('id');}else{grid_item=$('li.'+book.slug);next=grid_item.nextAll(':visible').first();previous=grid_item.prevAll(':visible').first();if(next.length===0)next=null;else next=next.attr('data-slug');if(previous.length===0)previous=null;else previous=previous.attr('data-slug');}$modal_content.append(JST.book_modal({book:book,next:next,previous:previous,SMALL_MOBILE:(SMALL&&MOBILE)}));$modal.scrollTop(0);$modal.modal();$back_to_top.hide();};var on_hash_changed=function(new_hash,old_hash){var bits=new_hash.split('/');var hash_type=bits[0];var hash_slug=bits[1];if(hash_type=='tag'){$modal.modal('hide');on_tag_hash(hash_slug);}else if(hash_type=='book'){on_book_hash(hash_slug);$modal.show().css('overflow-y','hidden').scrollTop(0).css('overflow-y','scroll');if(first_hash)filter_books();}else{$modal.modal('hide');selected_tags=[];filter_books();}if(new_hash=='_')new_hash='';_gaq.push(['_trackPageview',location.pathname+'#'+new_hash]);first_hash=false;return false;};var on_book_modal_closed=function(){if(selected_tags.length>0)hasher.setHash('tag',selected_tags.join(','));else hasher.setHash('_');return true;};var relayout=_.throttle(function(){$books_grid.isotope('reLayout');},500);var unveil_grid=function(){$books_grid.find('img').unveil(800,function(){$(this).load(function(){relayout();});});};var toggle_filter_modal=function(){$filter.toggleClass('hidden-xs').scrollTop(0);};var toggle_books_list=function(){$books_grid.toggle();$books_list.toggle();$toggle_text.toggleClass('grid-active list-active');if($books_grid.is(':visible'))isotope_grid();};var nypl_banner_start=function(){window.setInterval(function(){nypl_banner_next();},7000);};var nypl_banner_next=function(){var $current=$('.header-image.active').first(),$all=$('.header-images').children('.header-image'),current_pos=$all.index($current),next_pos=current_pos+1,$next;if(next_pos>=$all.length)next_pos=0;$next=$all.eq(next_pos);$all.removeClass('active');$next.addClass('active');};$(function(){$body=$('body');$content=$('#content');$books_grid=$('#books-grid');$all_tags=$('.tags .tag');$clear_tags=$('.clear-tags');$current_tag=$('.current-tag');$modal=$('#myModal');$modal_content=$('#myModal .modal-content');$books_list=$('#books-list');$back_to_top=$('#back-to-top');$mobile_filters_btn=$('#mobile-filters');$filter=$('.filter.tags');$show_text_button=$('.show-text');$show_books_button=$('.show-books');$toggle_text=$('.toggle-text');$review=$('.review');$body.on('click','.filter .tag',on_tag_clicked);$content.on('click','.back-to-top',back_to_top);$content.on('click','button.clear-tags',on_clear_tags_clicked);$modal.on('hidden.bs.modal',on_book_modal_closed);$modal.on('click','.tag',on_modal_tag_clicked);$back_to_top.on('click',back_to_top);$mobile_filters_btn.on('click',toggle_filter_modal);$filter.find('.close-modal').on('click',toggle_filter_modal);$toggle_text.on('click',toggle_books_list);$(window).on('scroll',on_page_scroll);$modal.keyup(function(e){if($('#myModal:visible').length>0)if(e.which===37&&previous!==null)hasher.setHash('book',previous);else if(e.which===39&&next!==null)hasher.setHash('book',next);});$back_to_top.hide();$current_tag.find('#showing-span').text('Showing all books');$current_tag.show();_.each($all_tags,function(tag){var $tag=$(tag);$tags[$tag.data('tag-slug')]=$tag;});if(MOBILE){$books_grid.addClass('no-transition');$review.remove();}else $body.append('<style>.book.card { width: 250px; }</style>');hasher.changed.add(on_hash_changed);hasher.initialized.add(on_hash_changed);hasher.init();hasher.setHash('_');_.delay(unveil_grid,1000);nypl_banner_start();});